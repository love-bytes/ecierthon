/*
** $Id: ecierthonc.c $
** ecierthon compiler (saves bytecodes to files; also lists bytecodes)
...
#define ecierthonU_print	PrintFunction

#define PROGNAME	"ecierthonc"		/* default program name */
...
static const char* reader(ecierthon_State* L, void* ud, size_t* size)
{
 UNUSED(L);
 if ((*(int*)ud)--)
 {
...
static const Proto* combine(ecierthon_State* L, int n)
{
 if (n==1)
  return toproto(L,-1);
 else
 {
  Proto* f;
  int i=n;
  if (ecierthon_load(L,reader,&i,"=(" PROGNAME ")",NULL)!=ecierthon_OK) fatal(ecierthon_tostring(L,-1));
  f=toproto(L,-1);
  for (i=0; i<n; i++)
  {
...
static int writer(ecierthon_State* L, const void* p, size_t size, void* u)
{
 UNUSED(L);
...
static int pmain(ecierthon_State* L)
{
 ...
 int i;
 tmname=G(L)->tmname;
 if (!ecierthon_checkstack(L,argc)) fatal("too many input files");
 for (i=0; i<argc; i++)
 {
  const char* filename=IS("-") ? NULL : argv[i];
  if (ecierthonL_loadfile(L,filename)!=ecierthon_OK) fatal(ecierthon_tostring(L,-1));
 }
 f=combine(L,argc);
 if (listing) ecierthonU_print(f,listing>1);
 if (dumping)
 {
  FILE* D= (output==NULL) ? stdout : fopen(output,"wb");
  if (D==NULL) cannot("open");
  ecierthon_lock(L);
  ecierthonU_dump(L,f,writer,D,stripping);
  ecierthon_unlock(L);
...
int main(int argc, char* argv[])
{
 ...
 L=ecierthonL_newstate();
 if (L==NULL) fatal("cannot create state: not enough memory");
 ecierthon_pushcfunction(L,&pmain);
 ecierthon_pushinteger(L,argc);
 ecierthon_pushlightuserdata(L,argv);
 if (ecierthon_pcall(L,2,0,0)!=ecierthon_OK) fatal(ecierthon_tostring(L,-1));
 ecierthon_close(L);
 return EXIT_SUCCESS;
}
...
static void PrintConstant(const Proto* f, int i)
{
 ...
 switch (ttypetag(o))
 {
  ...
  case ecierthon_VNUMFLT:
	{
	char buff[100];
	sprintf(buff,ecierthon_NUMBER_FMT,fltvalue(o));
	printf("%s",buff);
	if (buff[strspn(buff,"-0123456789")]=='\0') printf(".0");
	break;
	}
  case ecierthon_VNUMINT:
	printf(ecierthon_INTEGER_FMT,ivalue(o));
	break;
  case ecierthon_VSHRSTR:
  case ecierthon_VLNGSTR:
...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  int isk=GETARG_k(i);
  int line=ecierthonG_getfuncline(f,pc);
  printf("\t%d\t",pc+1);
